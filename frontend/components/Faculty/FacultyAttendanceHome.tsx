...
export default function FacultyAttendanceHome({ user, token, apiBaseUrl }: Props) {
  ...
  const fetchAttendance = useCallback(async (isRefresh = false) => {
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/api/attendance?refresh=${Date.now()}`, {
      ...
      if (!response.ok) {
        setErrorMessage(payload?.error ?? "Failed to load attendance.");
        return;
      }

      const latest = payload?.attendance?.[0];
      if (!latest) {
        setAttendance(null);
      ...
      const loginTime = latest.login_time;
      const logoutTime =
      ...
      setAttendance({
        ...
        totalHoursLabel: formatDuration(loginTime, logoutTime),
      });
    } catch {
          ...
          {!hasAttendanceData ? (
            ...
            <View style={styles.detailsTable}>
              ...
              <View style={styles.detailItem}>
                ...
                <Text style={styles.detailValue}>{attendance?.totalHoursLabel ?? "--"}</Text>
              </View>
            </View>
          )}
        </View>
